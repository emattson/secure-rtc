...
describe('secure-rtc', function () {
  this.timeout(4000); //20 seconds callback timeout cause pgp can be slow
  ...
  describe('webrtc', function () {
    ...
    it('should tell whether webRTC is supported', function () {
      expect(Peer.WEBRTC_SUPPORT).to.exist;
    });
  ...
  describe('pgp key generation and exporting', function () {
    ...
    it('should be able to export its public key', function (done) {
      ...
      setTimeout(function () {
        ...
        expect(peer).to.have.property('publicKey')
            .that.is.a('string');
        cleanup(peer);
        done();
      }, 2000);
